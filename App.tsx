...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        const current =
          (await authService.getCurrentUser()) || authService.getStoredUserQuick();
        if (!isMounted) return;
    ...
    try {
      ...
      const timeout = (label: string) =>
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error(`${label} timed out`)), 12000)
        );
  ...
  // ✅ Change language AND update URL (/de, /en, /sr)
  const handleChangeLanguage = (code: string) => {
    setLanguage(code as any);

    // keep current path after language prefix
    // examples:
    //  /de           -> /en
    //  /de/something -> /en/something
    const parts = location.pathname.split('/').filter(Boolean);
    if (parts.length > 0) parts[0] = code; // replace current lang segment
          ...
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="md:hidden flex items-center gap-4 relative">
              ...
              {user ? (
                <div
                  onClick={handleOpenHistory}
                  className="w-8 h-8 rounded-full bg-primary-100 text-primary-700 flex items-center justify-center text-xs font-bold cursor-pointer"
                >
                  {user.name.charAt(0).toUpperCase()}
                </div>
              ) : (
              ...
              <button className="text-slate-600" onClick={handleOpenHistory}>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
                  <div className="flex items-center gap-3 text-sm text-amber-900">
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
                      ...
                      <span>
                        {activeReminders[0].pickupLocation}{' '}
                        <i className="fa-solid fa-arrow-right text-xs"></i>{' '}
                        {activeReminders[0].dropoffLocation}
                      </span>
                    </div>
                    <button
                      onClick={handleOpenHistory}
                  ...
                  <div className="flex justify-between items-start border-t border-slate-100 pt-3 mt-3">
                    <span className="text-slate-500 mt-1">
                      {bookingComplete.pickupLocation === VIENNA_AIRPORT
                        ? t('label.destinationAddress')
                        : `${t('label.pickup')} ${t('label.address')}`}
                    </span>
                ...
                <div className="flex justify-between border-t border-slate-100 pt-3 mt-3">
                  ...
                  <div className="text-right">
                    <span className="font-bold text-slate-800 block text-lg">
                      {formatDate(bookingComplete.date)}
                    </span>
                    <span className="text-slate-600 text-sm block">
                      {bookingComplete.pickupLocation === VIENNA_AIRPORT
                        ? t('label.landingTime')
                        : t('label.pickupTime')}{' '}
                      {bookingComplete.time}
                    </span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div className="col-span-1 md:col-span-2">
              ...
              <p className="text-slate-400 max-w-sm text-sm">{t('footer.desc')}</p>
            </div>
            <div>
              <h4 className="text-lg font-bold mb-4">{t('footer.contact')}</h4>
              <ul className="space-y-2 text-slate-400 text-sm">
                <li>
                  <i className="fa-solid fa-phone mr-2 text-primary-500"></i> +43 1 234 5678
                </li>
                <li>
                  <i className="fa-solid fa-envelope mr-2 text-primary-500"></i> booking@vieride.com
                </li>
                <li>
                  <i className="fa-solid fa-location-dot mr-2 text-primary-500"></i> Vienna Airport, Office 23
                </li>
              </ul>
            </div>
            <div>
